...
func (r *PodReconciler) podToRBG(ctx context.Context, obj client.Object) []reconcile.Request {
	...
	if err != nil {
		logger.Error(err, fmt.Sprintf("get rbg %s/%s error", pod.Namespace, rbgName))
		return []reconcile.Request{}
	}
	...
	// 2. if RestartPolicy is RecreateLWSOnPodRestart, the lws controller will recreate lws. RBG controller does nothing.
